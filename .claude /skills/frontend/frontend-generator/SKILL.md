...
description: Reverse-engineer and clone a website from one page URL plus its sitemap URL — extracts assets, CSS, and content section-by-section and proactively dispatches parallel builder agents in worktrees as it goes. Use this whenever the user wants to clone, replicate, rebuild, reverse-engineer, or copy any website. Also triggers on phrases like "make a copy of this site", "rebuild this page", "pixel-perfect clone". Provide exactly two arguments: target URL and sitemap URL.
...
You are about to reverse-engineer and rebuild the site defined by **$ARGUMENTS** as a pixel-perfect multi-page clone.

This skill requires EXACTLY TWO arguments:

- `$ARGUMENTS[0]` = Canonical page URL on the target site (used as origin and baseline)
- `$ARGUMENTS[1]` = Sitemap URL

If argument count is not exactly 2, stop and ask the user to provide:
`clone-website <url> <sitemap-url>`

Execution contract:

1. Fetch and parse the sitemap URL
2. Extract all page URLs from sitemap (and nested sitemaps, if present)
...
5. Treat EACH remaining URL as a required page in the clone
6. Repeat the full pipeline (recon -> extract -> build -> assemble) for EACH page
7. Reuse shared components (navbar, footer, design tokens, icons) across pages
8. Build a complete multi-page output; do not stop after a single page

This is not a two-phase process (inspect then build). You are a **foreman walking the job site** — as you inspect each section of the page, you write a detailed specification to a file, then hand that file to a specialist builder agent with everything they need. Extraction and construction happen in parallel, but extraction is meticulous and produces auditable artifacts.

## Scope Defaults

The target scope is all in-scope pages discovered from `$ARGUMENTS[1]` (sitemap URL). `$ARGUMENTS[0]` defines canonical domain/origin and acts as the seed page for initial foundation extraction. Unless the user specifies otherwise, use these defaults:

- **Fidelity level:** Pixel-perfect — exact match in colors, spacing, typography, animations
- **In scope:** Visual layout and styling, component structure and interactions, responsive design, mock data for demo purposes
- **Out of scope:** Real backend / database, authentication
...
2. Validate arguments strictly:
   ...
   - `$ARGUMENTS[1]` must be a valid sitemap URL
   - Both must be reachable via browser MCP
   - Sitemap pages must resolve to the same hostname as `$ARGUMENTS[0]` unless user explicitly allows cross-domain pages
3. Always create a NEW dedicated project under the `/project` directory:
   `project/<project-name>/`
   where `<project-name>` should be a stable slug derived from the target hostname (or user-provided name).
   Use:
   `npx create-next-app@latest project/<project-name>`
   Ensure: TypeScript + Tailwind + App Router
4. Set `project/<project-name>/` as the working directory for all subsequent commands, files, docs, scripts, and artifacts.
...
6. Create the output directories inside `project/<project-name>/`:
   - `docs/research/`
   - `docs/research/components/`
   - `docs/research/pages/`
   - `docs/design-references/`
   - `docs/design-references/pages/`
   - `scripts/`
   - `src/components/shared/`
   - `src/components/pages/`
7. Build page mapping from sitemap URL to Next.js routes:
   - `/` -> `src/app/page.tsx`
   - `/about` -> `src/app/about/page.tsx`
   - `/products/item-a` -> `src/app/products/item-a/page.tsx`
     Keep this mapping in `docs/research/SITEMAP_PAGE_MAP.md`.
...
### 10. Next.js Best Practices Are Mandatory

Treat performance, static generation, and SEO as first-class requirements for every cloned page. By default, prefer Server Components, statically generated routes, and metadata-driven SEO. Only opt into client-side rendering or dynamic behavior when interaction requirements force it.

## Phase 1: Reconnaissance

Navigate first to `$ARGUMENTS[0]` with browser MCP, then perform per-page recon for every URL discovered from the sitemap.

### Screenshots

- Take **full-page screenshots** at desktop (1440px) and mobile (390px) viewports
- Save to `docs/design-references/pages/<route-slug>/` with descriptive names
- Required minimum per page:
  - `desktop-full.png`
  - `mobile-full.png`
- Also write `docs/design-references/pages/<route-slug>/manifest.json` listing generated files
- These are your master reference — builders will receive section-specific crops/screenshots later
- If any page has no screenshot files, STOP and fix capture before continuing
...
Save findings inside `project/<project-name>/` to:

- `docs/research/BEHAVIORS-global.md` for global/shared behavior
- `docs/research/pages/<route-slug>/BEHAVIORS.md` for page-specific behavior
  These are your behavior bibles — reference them when writing every component spec.
...
Save this inside `project/<project-name>/` as:

- `docs/research/PAGE_TOPOLOGY-global.md` for shared shell/layout
- `docs/research/pages/<route-slug>/PAGE_TOPOLOGY.md` for each page
  These become your assembly blueprints.

## Phase 2: Foundation Build

This is sequential and global. Do it yourself (not delegated to an agent) since it touches many files shared by all sitemap pages:
...
This is the core loop. For each page from the sitemap, and for each section in that page's topology (top to bottom), do THREE things: **extract**, **write the spec file**, then **dispatch builders**.
...
1. **Screenshot** the section in isolation (scroll to it, screenshot the viewport). Save to `docs/design-references/`.
   Use page-scoped folder:
   `docs/design-references/pages/<route-slug>/sections/<section-name>.png`
...
For each section (or sub-component, if you're breaking it up), create a spec file in `docs/research/components/` under `project/<project-name>/`. This is NOT optional — every builder must have a corresponding spec file.

**File path:** `docs/research/components/<route-slug>--<component-name>.spec.md`
...
## Phase 4: Multi-Page Assembly

After all sections are built and merged, wire everything together for EVERY sitemap page inside `project/<project-name>/`:

- Build shared shell/layout components in `src/components/shared/`
- Build page-specific sections in `src/components/pages/<route-slug>/`
- Assemble each route at its mapped Next.js file (`src/app/**/page.tsx`)
- Import all relevant section components per route
- Implement page-level layout from each route topology doc (scroll containers, column structures, sticky positioning, z-index layering)
- Connect real content to component props
- For API/CMS-backed runs: frontend-rendered copy and media must match CMS/source content exactly (no placeholder/lorem/paraphrased text)
...
- Verify: `npm run build` passes clean for the full multi-page app

## Phase 5: Visual QA Diff

After assembly, do NOT declare the clone complete. Take side-by-side comparison screenshots for EACH sitemap page:
...
Only after this visual QA pass across all sitemap pages is the clone complete.

## Next.js Static Generation and SEO Checklist

Apply this checklist across all routes in `project/<project-name>/`:

1. **Default to static output for page routes.**
   - Keep route pages as Server Components unless browser-only APIs are required.
   - Use static content extraction to render pages at build time whenever possible.
   - For dynamic segment routes discovered in the sitemap, implement `generateStaticParams()` so all known paths are pre-rendered.
   - If a route must refresh content periodically, use ISR via `export const revalidate = <seconds>` rather than full runtime rendering.

2. **Keep client JavaScript minimal.**
   - Add `"use client"` only to leaf interactive components, not entire page trees.
   - Avoid wrapping static sections in client components.
   - Pass precomputed props from server components to client islands.

3. **Implement route-level SEO metadata with the Metadata API.**
   - Define `metadata` or `generateMetadata` in each route/layout with page-specific `title`, `description`, and canonical URL.
   - Configure `metadataBase` in root layout for consistent canonical generation.
   - Include Open Graph and Twitter metadata for share previews.
   - Set `robots` directives explicitly (index/follow where appropriate).

4. **Ship crawlability primitives.**
   - Create `src/app/sitemap.ts` that reflects all cloned pages from the sitemap map.
   - Create `src/app/robots.ts` with sane defaults and sitemap location.
   - Ensure internal links are real `<a>` links through Next.js navigation patterns (no click-only div navigation).

5. **Optimize media and content semantics.**
   - Use `next/image` for raster images with accurate `alt` text and `sizes`.
   - Mark the LCP/hero image with `priority` when needed.
   - Preserve heading hierarchy (`h1` once per page where appropriate, then ordered `h2/h3`).
   - Prefer semantic landmarks (`header`, `main`, `nav`, `section`, `footer`) to improve accessibility and SEO interpretation.

6. **Include structured data where obvious.**
   - Add JSON-LD for organization, website, breadcrumb, or article/product schema when the source page clearly represents those entities.
   - Validate schema format and keep values aligned with visible page content.

7. **Verify static and SEO output before completion.**
   - Run `npm run build` and inspect build output for expected static pre-rendered routes.
   - Spot-check rendered `<head>` content in the browser for canonical/meta/OG tags.
...
- [ ] For CMS-connected pages, rendered UI content matches Strapi response values exactly
- [ ] The builder prompt is under ~150 lines of spec; if over, the section needs to be split
- [ ] Page-level screenshot files exist in `docs/design-references/pages/<route-slug>/`
- [ ] Section screenshots exist in `docs/design-references/pages/<route-slug>/sections/`
...
- Total sitemap pages discovered
- Total sitemap pages cloned
- Total sections built
- Total components created
- Total spec files written (should match components)
- Total assets downloaded (images, videos, SVGs, fonts)
- Total page screenshots generated
- Total section screenshots generated
- Screenshot artifact root (`docs/design-references/pages/`)
- Build status (`npm run build` result)
- Visual QA results by page (any remaining discrepancies)
- Project output path (`project/<project-name>/`)
