...
1. `.claude/skills/PA-reverse-engineering/site-crawler/SKILL.md` — full-site crawl, DOM, routes, media, sitemaps
2. `.claude/skills/PA-reverse-engineering/wp-source-adapter/SKILL.md` — WordPress REST API extraction (optional, only if WP detected)
3. `.claude/skills/PA-reverse-engineering/content-model-inferencer/SKILL.md` — AI-powered content model inference
...
2. Validate and normalise the URL argument (add `https://` if scheme is missing). If missing, stop and ask exactly: `/PA-reverse-engineering <legacy-site-url> [sitemap-url]`
...
Phase A is a standalone phase. When invoked as `/PA-reverse-engineering`, it completes at `APPROVED.md` and stops.

Phase B is only triggered by:

- The `url-to-strapi` orchestrator (which runs Phases A → B → C → D → E in sequence)
- Explicit user invocation: `/PB-test`
