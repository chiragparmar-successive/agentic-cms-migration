...
2. Validate the site slug argument. If missing, stop and ask: `PE-quality <site-slug>`
...
- Validate frontend visual/content match against approved baseline input artifacts (screenshots + text snapshots) for P0/P1 routes
...
Mandatory failure report when max iterations are hit:

- Write `output/<site>/test/reports/FAILED-AFTER-5-ITERATIONS.md` including:
  - failing gate(s) and final status
  - iteration-by-iteration actions attempted
  - root-cause summary
  - impacted routes/tests and screenshot references
...
- Path to failure report when max-iteration exhaustion occurs: `output/<site>/test/reports/FAILED-AFTER-5-ITERATIONS.md`
...
Phase E is a standalone phase. When invoked as `/PE-quality`, it completes at `DEPLOYMENT-READY.md` and stops.
...
- [ ] If any gate still fails after 5 iterations, `output/<site>/test/reports/FAILED-AFTER-5-ITERATIONS.md` is written
