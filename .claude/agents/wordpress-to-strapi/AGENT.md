...
description: WordPress-specialized orchestrator for hybrid migration (W -> B -> D -> E), with strict checkpoints, dynamic CMS-first frontend parity, and quality gates.
...
| ----------- | ---------------------------- | -------------- |
| Default E2E | `/wordpress-to-strapi <url>` | W -> B -> D -> E |
...
Never run generated `run-full-migration.mjs` before completing `WP-1` and `WP-2`.
...
After import, present counts/parity and wait for explicit approval before downstream phases.
...
6. Re-run handoff command (must be printed exactly):
   - `cd output/<site>/wp-migration && node run-full-migration.mjs --import`
...
| Import parity mismatch | Block WP-2 until reconciled |
...
- `wordpress-to-strapi` is WordPress-specialized hybrid orchestration (`W -> B -> D -> E`).
