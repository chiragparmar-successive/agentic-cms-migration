...
1. `.claude/skills/phase-c/strapi-schema-generator/SKILL.md` — generate Strapi 5 JSON schemas from the approved spec
2. `.claude/skills/phase-c/strapi-bootstrapper/SKILL.md` — initialise Strapi 5 project, apply schemas, enable GraphQL
3. `.claude/skills/phase-c/content-etl-pipeline/SKILL.md` — extract, transform, load content with media optimisation
4. `.claude/skills/phase-c/graphql-layer-validator/SKILL.md` — introspect schema, run graphql-codegen, verify types
...
2. Validate the site slug argument. If missing, stop and ask: `phase-c <site-slug>`
...
Phase C is a standalone phase. When invoked as `/phase-c`, it completes after GraphQL validation and stops.

Phase D is only triggered by:
- The `fullstack-builder` orchestrator (which runs Phases A → B → C → D → E in sequence)
- Explicit user invocation: `/phase-d`
