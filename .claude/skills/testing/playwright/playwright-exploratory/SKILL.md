...
context.on('page', async (newPage) => {
  ...
  await newPage.screenshot({ path: `baseline/${slug}/new-window-screenshot.png`, fullPage: true });
...
```ts
test(`[crawl] ${slug} — new-window link "${linkText}" opens and loads`, async ({ page, context }) => {
  await page.goto(url);
