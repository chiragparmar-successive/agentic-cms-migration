...
async function writeTypeFiles(apiRoot, typeDef) {
  ...
  const base = path.join(apiRoot, typeDef.apiId, "content-types", uid);
  ...
  await fs.writeFile(
    path.join(apiBase, "controllers", `${typeDef.apiId}.ts`),
    controllerTs(uid),
  );
  await fs.writeFile(
    path.join(apiBase, "routes", `${typeDef.apiId}.ts`),
    routerTs(uid),
  );
  await fs.writeFile(
    path.join(apiBase, "services", `${typeDef.apiId}.ts`),
    serviceTs(uid),
  );
}
...
function enrichContentType(ct) {
  ...
  const fields = [
    ...SYNC_FIELDS.map((f) => ({ ...f })),
    ...(ct.fields || []).filter(
      (f) => !SYNC_FIELDS.some((s) => s.name === f.name),
    ),
  ];
  ...
  return {
    ...
    singularName,
    kind:
      ct.kind === "singleType" ? "collectionType" : ct.kind || "collectionType",
    fields,
  };
}
...
export async function generateStrapiSchemas(siteSlug) {
  ...
  const manifest = {
    meta: {
      ...
      siteSlug,
      schemaRoot: apiRoot,
      note: "Restart Strapi after applying schemas. Partial: wordpress-to-strapi.mjs --import. Full data: output/<site>/wp-migration/run-full-migration.mjs --import.",
    },
