 ...
 * Full: /wp-to-strapi-db-migration — extract + normalize only (no content modeling)
 *
 * Usage:
...
async function main() {
  ...
  for (const s of steps) {
    if (s === "extract") {
      ...
      console.log(
        "[normalize] OK",
        result.paths.normalizedFile,
        `items=${result.itemCount}`,
      );
    } else if (s === "detect") {
      if (profileId === "full") {
        console.warn(
          "[detect] skipped for full profile — use preview analysis for schemas",
        );
        continue;
      }
