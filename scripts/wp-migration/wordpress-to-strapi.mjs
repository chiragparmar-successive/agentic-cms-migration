...
import { initSiteConfig, syncSiteConfigFromAnalysis, generateFullMigrationRunner } from './lib/site-config.mjs';
...
if (!siteSlug || !wpUrl) {
  console.error('Usage: node scripts/wp-migration/wordpress-to-strapi.mjs <site-slug> <wordpress-url> [--import] [--cms-only] [--skip-tests]');
  process.exit(1);
}
...
`);
  ...
  if (!doImport) {
    console.log(`${LOG} CMS scripts done (Phase W only — no frontend in this script).`);
    console.log(`  Restart Strapi, then: node scripts/wp-migration/wordpress-to-strapi.mjs ${siteSlug} ${wpUrl} --import`);
    console.log(`  Full data later: node output/${siteSlug}/wp-migration/run-full-migration.mjs --import`);
...
function printPostWInstructions(siteSlug, flags) {
  if (flags.cmsOnly) {
    console.log(`${LOG} --cms-only: skip Phase B (tests), D (frontend), E (quality). Done.`);
    return;
  }
  console.log('');
  console.log(`${LOG} Next — orchestrator must run (not this script):`);
  if (flags.runTests) {
    console.log('  Phase B: .claude/commands/phase-b.md');
  }
  if (flags.runFrontend) {
    console.log('  Phase D: nextjs-scaffolder → cms-adapter-generator → page-component-generator → route-validator');
    console.log(`       → creates output/${siteSlug}/frontend/`);
  }
  if (flags.runQualityGates) {
    console.log('  Phase E: playwright-behavioral-parity, sonarqube-gate, lighthouse-ci-gate');
  }
}
