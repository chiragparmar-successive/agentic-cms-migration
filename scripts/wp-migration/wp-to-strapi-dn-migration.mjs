#!/usr/bin/env node
/**
 * /wp-to-strapi-dn-migration — Data-only full migration.
 *
...
const LOG = '[wp-to-strapi-dn-migration]';
...
if (!siteSlug || !wpUrl) {
  console.error(
    `Usage: node scripts/wp-migration/wp-to-strapi-dn-migration.mjs <site-slug> <wordpress-url> [--import]`
  );
  process.exit(1);
}
...
async function main() {
  ...
  if (!doImport) {
    ...
    console.log(
      `  STRAPI_URL=... STRAPI_API_TOKEN=... node scripts/wp-migration/wp-to-strapi-dn-migration.mjs ${siteSlug} ${wpUrl} --import`
    );
    return;
  }
